...
class Finance extends CI_Controller {
	...
	function finance_all_money_report(){
		...
		// ไม่เรียกเก็บเลย
		//----------------
		$where .= " AND mem_type NOT IN (7, 8, 99) AND faction NOT IN (2324,2382,2440,2498)";
		//----------------
		...
		$member_date_limit = date('Y-m-t', strtotime(($_GET['year']-543)."-".$_GET['month']."-01"));
		$where .= " AND coop_mem_apply.member_date <= '{$member_date_limit}' ";


		if(@$_GET['run_current']!=""){
			$this->db->limit( @$_GET['limit'], ($_GET['run_current']-1)*@$_GET['limit'] );
		}
		$row_member = $this->db->select(array('coop_mem_apply.member_id','coop_mem_apply.apply_type_id','coop_mem_apply.share_month','coop_prename.prename_short','coop_mem_group.mem_group_name','coop_mem_apply.mem_type','coop_mem_apply.mem_type_id'))
			->from('coop_mem_apply')
		...
		$this->load->model("finance_process_model", 'fpm');
		...
		foreach($row_member as $key_member => $row){
			...
			$date_interesting = $this->fpm->get_day($row['mem_type_id'], $date_month_end, $row['member_id']);
			if($date_interesting === "N/A" || $date_interesting === NULL){
				continue;
			}
				...
				if($value2['deduct_code']=='OTHER'){
					...
					foreach($row_loan as $key => $row_normal_loan){
						...
						if(!empty($ref_id)){
							...
							if(!empty($rs_period)){
								foreach($rs_period AS $index => $row_period){
							...
							$date_interesting = $this->fpm->get_day($row['mem_type_id'], $date_month_end);
							$date_last_cal = $this->fpm->find_last_cal_interest($for_loan_id, $date_month_start);
							...
							$interest = $this->loan_libraries->calc_interest_loan_type_with_loan_and_member_id($row_normal_loan['loan_amount_balance'], $row_normal_loan['loan_type'], $row_normal_loan['date_last_interest'], $date_interesting, $row['member_id'], $cal_loan_interest['loan_id']);
							...
							//echo $row_normal_loan['id']." ".$this->Interest_modal->getCmpDateLastInterest($row_normal_loan['id'], $row_normal_loan['date_last_interest']);
							//echo "<br>";
							...
							$LOAN[$j]['money_per_period'] = ($row_normal_loan['money_per_period']!="") ? $row_normal_loan['money_per_period'] : $row_normal_loan['money_period_1'];
							$LOAN[$j]['money_period_1'] = $row_normal_loan['money_period_1'];
							$LOAN[$j]['money_period_2'] = $row_normal_loan['money_period_2'];
							$LOAN[$j]['date_last_interest'] = $this->Interest_modal->getCmpDateLastInterest($row_normal_loan['id'], $row_normal_loan['date_last_interest']);
					...
					if(!empty($LOAN)){
						foreach($LOAN as $key3 => $value3){
							$refrain_type = $value3['refrain_type'];
							...
							if($value3['pay_loan_type'] == '2' && $value3['pay_type'] == 'principal'){
								...
								if(($value3['loan_amount_balance']) >= @$value3['money_period_1']){
									if($value3['check_month'] == 0){
										...
										if($this->Interest_modal->first_interest_payment($value3['loan_id'])){
											$date_last_interest = date("Y-m-d", strtotime($date_last_interest ." -1 DAY"));
										}

										$query_interest['money_period'] = $value3['money_per_period'];
										$query_interest['interest'] = $this->loan_libraries->calc_interest_loan_type_with_loan_and_member_id($value3['loan_amount_balance'], $value3['loan_type_id'], $date_last_interest, $date_target_interest, $row['member_id'], $value3['loan_id']);
										$pay_type = "";

										if($query_interest['pay_type']==1){ //จ่ายแบบคงต้น
											$data_arr[$p]['pay_amount'] = $query_interest['money_period'] - round($query_interest['interest'],0);
										}else{//จ่ายแบบคงยอด
											$data_arr[$p]['pay_amount'] = @$query_interest['money_period'] > $query_interest['interest'] ?  @$query_interest['money_period'] - round($query_interest['interest'], 0) : 0;
								...
								if($value3['check_month'] == 0 && $value3['pay_type'] != 'principal' && in_array($refrain_type, [0,1]) ){
									...
									$query_interest['money_period'] = $value3['money_per_period'];
									$query_interest['interest'] = $this->loan_libraries->calc_interest_loan_type_with_loan_and_member_id($value3['loan_amount_balance'], $value3['loan_type_id'], $date_last_interest, $date_target_interest, $row['member_id'], $value3['loan_id']);
									$data_arr[$p]['pay_amount'] = round($query_interest['interest'], 0) ;
					...
					//ทุนเรือนหุ้น
					if(in_array(@$row['apply_type_id'], [1,2,3, 4]) && !in_array($row['mem_type'], [4]) && $check_refrain_share == 0 && @$row['share_month'] > 0){
					...
					//หักส่งรายเดือน
					$rs_deposit_month = $this->db->select(array('t1.account_id','t1.total_amount','t1.deduction_type','DATE(t2.created) AS date_open','t1.deduction_month'))
						...
						->get()->result_array();
//					echo"<pre>";print_r($rs_deposit_month );echo"</pre>";exit;
//					echo $this->db->last_query(); exit;
//					echo "<hr>";
					$deposit_month = $rs_deposit_month[0];
					$deposit = array();
					foreach($rs_deposit_month as $deposit_month) {
						if($deposit_month['deduction_type']=='0' && !in_array($deposit_month['account_id'], $deposit)){
							$data_arr[$p]['deposit_account_id'] = $deposit_month['account_id'];
							$data_arr[$p]['pay_amount'] = $deposit_month['total_amount'];
							$data_arr[$p]['deduction_month'] = $deposit_month['deduction_month'];
							$sum_total += $deposit_month['total_amount'];
							$p++;
						}
						$deposit[] = $deposit_month['account_id'];
						//echo"<pre>";print_r($deposit_month);echo"</pre>";exit;
	...
	function finance_month_run_process(){
		...
		$this->db->select(array('member_id', 'mem_type_id'));
		...
		foreach($row_member as $key => $value){
			...
			$this->db->order_by("CAST(SUBSTR(receipt_id, 6, 6) as DECIMAL) DESC");
			$this->db->limit(1);
			$row_receipt = $this->db->get()->row_array();
			$sum_count = 0;
			$order_by_id =  $row_receipt["order_by"]+1;
			$pay_type = $this->receipt_model->getPayTypeByKeepingReceiptGroup($value['mem_type_id']);
			if(!empty($pay_type)) {
				$receipt_number = $this->receipt_model->generate_receipt($process_date, $pay_type);
			}else{
				$receipt_number = $this->receipt_model->generate_receipt($process_date, 8);
			}
				...
				foreach($row_process as $key_process => $value_process){
					...
					if($value_process['real_pay_amount']>0){
						...
						if($value_process['deduct_code'] == 'LOAN' || $value_process['deduct_code'] == 'GUARANTEE'){
							...
							$period_count = $this->loan_libraries->getPeriodCount($value_process['loan_id'], $process_date);
