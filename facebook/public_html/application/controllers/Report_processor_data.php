...
class Report_processor_data extends CI_Controller {
	...
	function coop_report_charged_department_preview(){
        ...
        foreach ($row as $key => $value){
            if($value['id'] == '8'){
                $new_row[] = $value;
                $new_row[$key+1]['id'] = '-';
                $new_row[$key+1]['loan_type'] = 'เงินกู้สามัญโครงการ';
                $new_row[$key+1]['loan_type_code'] = 'normal_project';
            }else{
                $new_row[] = $value;
            }
        }

        $arr_data['loan_type'] = $new_row;
	...
	function coop_report_charged_person_preview($arr_data) {
		...
		if ($_GET['term_of_loan']) {
			$loan_where .= " AND loan_type = ".$_GET['term_of_loan'];
		}
		...
		//Get Loan type info
		$loan_types = $this->db->select("coop_loan_type.id, coop_loan_type.loan_type_code, coop_loan_name.loan_name_id, coop_loan_name.loan_name_short")
								->from("coop_loan_name")
		...
		$member_groups = $this->db->select(array('t1.mem_group_id'
					...
					->from('(SELECT t1.id as `lv`, t2.id as `faction`, `t1`.`id`, `t1`.`mem_group_id`, t1.mem_group_name as mem_group_name 
		FROM coop_mem_group t1 inner join coop_mem_group t2 ON t1.mem_group_parent_id=t2.id) as t1')
					->join("(SELECT
					                t3.employee_id,
									t3.member_id,
									IF(t4.level_old IS NULL, t3.LEVEL, t4.level_old) AS LEVEL,
									IF(t4.faction_old IS NULL, t3.faction, t4.faction_old) AS faction,
									t3.firstname_th,
									t3.lastname_th,
									t3.prename_id,
									IF (
							...
							WHERE 1=1 AND member_status <> 3 ".$member_where.") as t2", "t1.id = t2.faction", "inner")
...
//                    ->limit(300)
		...
		foreach($member_groups as $key => $member_group){
			...
			foreach($member_indexs AS $member_index){
				...
				$datas[$member_group['member_id']]['mem_group_id'] = $member_group['mem_group_id'];
				...
				if($infoData['deduct_code']=='LOAN'){
					...
					$normal_project = array('14', '18', '26', '27'); // สามัญโครงการ
					if(in_array($infoData['loan_type'] , $normal_project)){
                        $loan_type_code = 'normal_project';
                    }
                    $loan_name_short = '';
					foreach ($loan_types as $loan_type_value){
                        if($infoData['loan_type'] == $loan_type_value['loan_name_id']){
                            $loan_name_short = $loan_type_value['loan_name_short'];
                        }
                    }
					...
					$datas[$member_group['member_id']][$loan_type_code][$infoData['loan_id']]['loan_name_short'] = $loan_name_short;
					...
					$this->db->select(array('loan_id','member_id'));
					$this->db->from('coop_loan_guarantee_compromise');
					$this->db->where("loan_id = '".$infoData['loan_id']."'");
					$chk_guarantee = $this->db->get()->result_array();
                    $suffix_contract = substr($infoData['contract_number'], -2);
                    $array_cpm = array('/1', '/2');

					if(!empty($chk_guarantee) || in_array( $suffix_contract,  $array_cpm)){
						$datas[$member_group['member_id']][$loan_type_code][$infoData['loan_id']]['chk_guarantee'] = 1;
					}else{
						$datas[$member_group['member_id']][$loan_type_code][$infoData['loan_id']]['chk_guarantee'] = '';
					}
