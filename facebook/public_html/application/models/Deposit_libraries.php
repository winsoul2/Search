...
class Deposit_libraries extends CI_Model {
	...
	public function cal_deposit_interest($row_member,$return_type = 'test_cal_interest', $date_interest = '', $day_interest = '', $param_date_dep = '', $from_holiday = false){
		...
		$this->db->where("t1.type_id = '".$row_member['type_id']."' AND t1.start_date <= '".$date_interest."'");
		...
		//เช็คข้อมูล ไม่คิดดอกเบี้ยในวันที่ทำที่มีรายการถอน
		$chk_is_non_pay_interest_after_withdraw = $this->chk_date_withdraw($row_member['account_id'],$date_interest);
		...
		if($row_detail['pay_interest'] == '1'){
			...
			//echo 'ext_num_month_maturity_day_AA='.$ext_num_month_maturity_day .'<br>';
			...
			*/
			...
			foreach($_rs->result_array() as $key => $value) {
				if(@$row_detail['is_withdrawal_specify'] == '1') {
					...
					if(@$row_balance_last_ref['balance_deposit'] == 0){
						continue;
					}
			...
			if($fix_saving) {
				...
				foreach($row_interest_rate as $key_rate => $value_rate){
					...
					if($value_rate['end_date'] != $date_interest && $date_interest > $value_rate['end_date']){
						...
						//$this->db->where("account_id = '{$row_member['account_id']}' AND transaction_time < '{$value_rate['end_date']}' AND YEAR(transaction_time) = ".date("Y",strtotime($date_interest)));
						$this->db->where("account_id = '{$row_member['account_id']}' AND transaction_time < '{$value_rate['end_date']} 23:59:59' AND YEAR(transaction_time) = ".date("Y",strtotime($date_interest)));
						$this->db->order_by("transaction_time DESC,transaction_id DESC");
						$this->db->limit(1);
				...
				foreach($transaction as $key => $value){
					...
					if($row_detail['pay_interest'] == '4'){
						//จ่ายเมื่อครบกำหนด  x เดือน (เงินฝากประจำ)
						$date_last_ext = date('Y-m-d', strtotime("+".$row_detail['ext_num_month_maturity_day']." day", strtotime($chk_date_ext_maturity)));
						if(($key + 1) == $transaction_count && ($key + 1) >= $row_detail['num_month_maturity'] && !empty($row_detail['ext_num_month_maturity_day']) && $date_interest > $date_last_ext) {
							$transaction_new[$key]['date_end'] = $date_last_ext;
...
//				echo "<pre>"; print_r($transaction_new); exit;
				...
				if($row_detail['pay_interest'] == '3') {
					...
					$this->db->order_by("transaction_time, transaction_id");
				}else if($row_detail['pay_interest'] == '1'){
					$this->db->select("DATE(transaction_time) AS transaction_date,transaction_time, transaction_list, transaction_deposit, transaction_withdrawal, transaction_balance,count(DATE( transaction_time )) AS c_data");
					$this->db->from("coop_account_transaction{$this->forecast}");
					$this->db->where("account_id = '".$row_member['account_id']."' AND transaction_list IN ('OPN', 'OPNX', 'OCA', 'OPT', 'TRB', 'XD', 'DEP', 'DCA', 'DFX', 'DEPP', 'CD', 'INT', 'IN', 'WTD', 'WTB', 'CW','DOT','WCQ','XW','YPF')
						AND YEAR(transaction_time) = YEAR('".$date_dep."')
						AND MONTH(transaction_time) = MONTH('".$date_dep."')".
						($is_prev_holiday ? " AND transaction_time > '".$prev_int_date." 23:59:59'" : ""));
					$this->db->order_by("transaction_time, transaction_id");
					$this->db->group_by("DATE(transaction_time)");
				}else {
					$this->db->select("transaction_time, transaction_list, transaction_deposit, transaction_withdrawal, transaction_balance");
					$this->db->from("coop_account_transaction{$this->forecast}");
				...
				if($row_detail['pay_interest'] == '1'){
					$chk_date = '';
					foreach($row as $_row){
						//เช็ครายการที่มีวันเดียวกัน  ให้ใช้ข้อมูลสุดท้ายของวัน
						if($_row['c_data'] > 1){
							$sql = "SELECT DATE(transaction_time) AS transaction_date, transaction_time, transaction_list, transaction_deposit, transaction_withdrawal, transaction_balance
									FROM coop_account_transaction
									WHERE account_id = '{$row_member['account_id']}' AND DATE(transaction_time) = '{$_row['transaction_date']}' ORDER BY transaction_time DESC LIMIT 1";
							$data_last = $this->db->query($sql)->row_array();
							
							if($transaction[$index - 1]['date_start'] == date('Y-m-d', strtotime($data_last['transaction_time'])) && $transaction[$index - 1]['type'] == "DEP") {
								if($this->debug) { echo $transaction[$index - 1]['date_start']."<br>"; }
								$transaction[$index - 1]['transaction_balance'] = $balance > 0 ? $balance : 0;
							}
							else {
								if($index > 0) {
									$transaction[$index - 1]['date_end'] = date('Y-m-d', strtotime($data_last['transaction_time']));
									$transaction[$index] = $transaction[$index - 1];
								}
								$transaction[$index]['type_id'] = $row_member['type_id'];
								$transaction[$index]['account_id'] = $row_member['account_id'];
								//$transaction[$index]['date_start'] = $data_last['transaction_list'] == "OPN" ||  $data_last['transaction_list'] == "OPNX" ? date('Y-m-d', strtotime("-1 day", strtotime($data_last['transaction_time']))) : date('Y-m-d', strtotime($data_last['transaction_time']));
								$transaction[$index]['date_start'] = date('Y-m-d', strtotime("-1 day", strtotime($data_last['transaction_time'])));
								$transaction[$index]['date_end'] = $date_interest;
								$transaction[$index]['transaction_balance'] = $data_last['transaction_balance'];
							...
							if($transaction[$index - 1]['date_start'] == date('Y-m-d', strtotime($_row['transaction_time'])) && $transaction[$index - 1]['type'] == "DEP") {
								...
								//เช็คข้อมูล ไม่คิดดอกเบี้ยในวันที่ทำที่มีรายการถอน กรณีปิดบัญชีเงินฝาก
								if($row_detail['is_non_pay_interest_after_withdraw'] == 1 && $this->close == true && $chk_is_non_pay_interest_after_withdraw == 1) {
									continue;
								}
								...
								$transaction[$index]['date_end'] = $date_interest;
								$transaction[$index]['transaction_balance'] = $_row['transaction_balance'];
								$transaction[$index]['type'] = 'DEP';
					...
					foreach($row  as $_row) {
						...
						if($_row["transaction_withdrawal"] != 0) {
							echo 'balance='.$balance.' - transaction_withdrawal='.$_row["transaction_withdrawal"].'<br>';
			...
			if($fix_saving) {
				...
				if($row_detail['pay_interest'] == '1'){
					$this->db->select("transaction_time, transaction_list, transaction_deposit, transaction_withdrawal, transaction_balance");
					$this->db->from("coop_account_transaction{$this->forecast}");
					$this->db->where("account_id = '".$row_member['account_id']."' AND transaction_list IN ('OPN','OPNX', 'OCA', 'OPT', 'TRB', 'XD', 'DEP', 'DCA', 'DFX', 'DEPP', 'CD', 'INT', 'IN', 'WTI', 'WTD', 'WTB', 'CW', 'WCQ', 'CM/FE', 'ERR','DOT','WCQ','XW','YPF')
		...
		foreach($transaction as $key => $value){
			...
			if($row_detail['type_interest'] == 1 && $row_detail['pay_interest'] == 1){
				if($key == 0 && sizeof($transaction) > 1){
					...
					$value['date_end'] = date("Y-m-d", strtotime($value['date_end']. " +0 day"));
				}
			}else {
			...
			//หาวันที่เปิดบัญชี หรือวันที่ครบกำหนดล่าสุด
			if(@$row_detail['is_withdrawal_specify'] == '1') {
				$create_account_date = $this->date_last_due($value['account_id'],$value['ref_account_no']);
			}else{
				$create_account_date = $value['create_account_date'];
			}
			
			$diff = @date_diff(date_create($value['date_start']),date_create($value['date_end']));
			$diff_opn = @date_diff(date_create($create_account_date),date_create($value['date_end']));
			//$date_count = @$diff->format("%a");
			...
			if($row_detail['type_fee'] == '3'){
				$arr_chk_fee = array();
				$arr_chk_fee['account_id'] = $value['account_id'];
				$arr_chk_fee['num_month_before'] = $row_detail['num_month_before'];
				$arr_chk_fee['ext_num_month_maturity_day'] = $row_detail['ext_num_month_maturity_day'];
				$arr_chk_fee['pay_interest'] = $row_detail['pay_interest'];
				$arr_chk_fee['num_month_maturity'] = $num_month_maturity;
				$arr_chk_fee['date_interest'] = $date_interest;
			}
			
			//เช็ควันของแต่ละปี 2 step
			if($row_detail['is_withdrawal_specify'] == 1) {
				//echo 'date_start='.$value['date_start'].',date_end='. $value['date_end'].'<br>';
				$array_check_date = self::check_fixed_deposit_list($value['account_id'], $value['date_start'], $value['date_end'], $value['ref_account_no']);
			}else{				
				$array_check_date = $this->check_interest_two_step($value['date_start'], $value['date_end'], $value['type_id'], $value['transaction_list'],$arr_chk_fee,$row_detail['main_type_id']);
			}
			//echo '=======array_check_date========';
			//echo '<pre>'; print_r($arr_chk_fee); echo '</pre>';
			//echo '<pre>'; print_r($array_check_date); echo '</pre>';
			$all_interest = 0;
			foreach($array_check_date AS $key_chk=>$val_chk){
				if($row_detail['type_interest'] == 1 && $row_detail['pay_interest'] == 6){
					if(@$row_detail['is_day_cal_interest'] == 1 && $this->close == true){
						//ลบ 1 วัน
						if($key === (sizeof($transaction)-1) && $key_chk === (sizeof($array_check_date)-1)){
							if($row_detail['main_type_id'] == 1){
								//บวก 1 วัน รายการสุดท้าย
								$date_count = @$val_chk['date_count']+1;
							}else if($row_detail['main_type_id'] == 6){
								//บวก 1 วัน รายการสุดท้าย
								$date_count = @$val_chk['date_count']+1;
							}else{
								//บวกลบ 1 วัน รายการสุดท้าย
								$date_count = @$val_chk['date_count']-1;
							}
						...
						if(@$row_detail['chk_date_cal_interest'] == '3'){
							...
							if($key == 0 && $key_chk == 0 && sizeof($transaction) > 1 && $value['transaction_list'] != 'OPN'){
								//ลบ 1 วัน
								$date_count = @$val_chk['date_count']-1;
							}else if(sizeof($transaction) === 1 && $value['transaction_list'] == 'OPN'){
								//บวกเพิ่ม 1 วัน
								$date_count = @$val_chk['date_count']+1;
							}else if($key === (sizeof($transaction)-1) && $key_chk === (sizeof($array_check_date)-1) && sizeof($transaction) > 1){
								//บวกเพิ่ม 1 วัน
								$date_count = @$val_chk['date_count']+1;
							}else if($value['transaction_list'] == 'OPN'){
								//ไม่บวกเพิ่ม
								$date_count = @$val_chk['date_count'];
							}else if($key == 0 && $key_chk == 0 && sizeof($transaction) == 1 && $value['transaction_list'] == 'IN'){
								//ลบ 1 วัน  รายการแรก
								$date_count = @$val_chk['date_count']-1;
							}else if($key === (sizeof($transaction)-1) && $key_chk === (sizeof($array_check_date)-1)){
								//บวกเพิ่ม 1 วัน รายการสุดท้าย
								$date_count = @$val_chk['date_count']+1;
							}
						}						
					}
				...
				//echo 'date_count_at_opn='.$date_count_at_opn.'<br>';
				//echo 'num_month_no_interest='.$num_month_no_interest.'<br>';
				//echo 'date_count_all='.$date_count_all.'<br>';
				//echo 'type_interest='.$row_detail['type_interest'].'<br>';
				if($row_detail['type_interest'] == '3' &&  $date_count_all < $num_month_no_interest && $date_count_at_opn < $num_month_no_interest){
					...
					if($row_detail['is_withdrawal_specify'] == 1){
						$interest = round(((($val_chk['transaction_balance'] * $interest_rate) / 100) * $date_count) / $day_of_year,2);
					}else {
				...
				if($fix_saving === false){
					...
					if($interest > 0 && $is_cal || $row_detail['pay_interest'] == '3'){
						...
						if(!($from_holiday && $row_detail['pay_interest'] == '1' && $row_detail['type_interest'] != '4')) {
							...
							//ดึงข้อมูลลำดับรายการ ของรายการถัดไป seq_no
							$arr_seq = array(); 
						...
						if($row_detail['type_interest'] == '4') {
							...
							if($row_acc["transfer_type"] == 1 && !empty($row_acc["dividend_acc_num"])) {
								...
								//ดึงข้อมูลลำดับรายการ ของรายการถัดไป seq_no
								$arr_seq = array(); 
								$arr_seq['account_id'] = $row_acc["dividend_acc_num"]; 
								$arr_seq['transaction_list'] = $data_insert['transaction_list'];
								$seq_no = $this->deposit_seq->gen_seq_account_transaction($arr_seq);
								$data_insert['seq_no'] = @$seq_no;
						...
						foreach($rs_due as $row_due) {
							...
							elseif($row_detail['condition_age'] == 3) {
								...
								if(!empty($row_acc["dividend_acc_num"])) {
									...
									//ดึงข้อมูลลำดับรายการ ของรายการถัดไป seq_no
									$arr_seq = array(); 
									$arr_seq['account_id'] = $data_insert2['account_id'];
									$arr_seq['transaction_list'] = $data_insert2['transaction_list'];
									$seq_no = $this->deposit_seq->gen_seq_account_transaction($arr_seq);
									$data_insert2['seq_no'] = @$seq_no;
						...
						if($interest > 0){
							...
							if(($row_detail['max_month'] == $adj_interest_period && !$is_holiday) || ($row_detail['max_month'] != $value['interest_period'] && $date_interest == $date_dep)  || ($row_detail['max_month'] == $value['interest_period'] && $date_interest == $date_dep && $row_detail['is_withdrawal_specify'] == '1')) {
								...
								if(@$row_detail['is_withdrawal_specify'] == '1') {										
									...
									//ดึงข้อมูลลำดับรายการ ของรายการถัดไป seq_no แบบเลือกยอด
									$arr_seq = array(); 
									$arr_seq['account_id'] = $account_id; 
									$arr_seq['transaction_list'] = $data_insert['transaction_list']; 
									$arr_seq['balance_deposit'] = $data_insert['balance_deposit']; 
									$seq_no = $this->deposit_seq->gen_seq_account_transaction($arr_seq);
									$data_insert['seq_no'] = @$seq_no;
						...
						if($row_detail['num_month_before'] > 0){
							...
							if($date_interest < $end_date){
								...
								if(@$row_detail['before_due_tax_rate'] > 0) {
									//echo 'interest_all='.$interest_all .'<br>';
									//ปิดไว้รถไฟไม่ได้ใช้แบบนี้
	...
	function check_interest_two_step($date_start,$date_end, $type_id, $checkList = array(), $arr_chk_fee = array(),$main_type_id=''){		
		$arr_data = array();
		$src_year = date("z", strtotime(substr($date_start, 0, 4)."-12-31")) + 1;
		$des_year = date("z", strtotime(substr($date_end, 0, 4)."-12-31")) + 1;
		
		//เช็คปิดบัญชีก่อนกำหนด
		if(!empty($arr_chk_fee)){
			$chk_fee = $this->check_close_account_premature($arr_chk_fee);
		}else{
			$chk_fee  = 0;
		}
		
		//เซ็ตวันที่ในการหาดอกเบี้ย ด้วยเว้นวันที่คิดดอกเบี้ยไปแล้ว
		if(in_array($checkList, array('IN')) && $date_start != $date_end){
		...
		if($src_year == $des_year){	
			...
			if($main_type_id == 1){
				$balance = 0;		
				$arr_multi_rate = $this->calc_multi_step_rate($type_id,$date_start, $date_end, $balance,$chk_fee);
				if(!empty($arr_multi_rate)){
					foreach($arr_multi_rate AS $key_rate=>$val_rate){
						$diff = @date_diff(date_create($val_rate['start_date']),date_create($val_rate['end_date']));
						$date_count = @$diff->format("%a");
						$arr_data[$key_rate]['date_count'] = $date_count;
						$arr_data[$key_rate]['day_of_year'] = $src_year;
						if($this->is_condition_step($date_start, $date_end, $type_id)){
							$this->db->where("type_id", $type_id);
							$this->db->where("start_date <=", $val_rate['start_date']);
							$this->db->order_by("start_date desc");
							$this->db->limit(1);
							$type_detail_id = $this->db->get_where("coop_deposit_type_setting_detail")->row_array()['type_detail_id'];
							
							if($chk_fee == 1){
								$field_interest_rate = "t1.percent_depositor as interest_rate";
							}else{
								$field_interest_rate = "t2.percent_interest as interest_rate";
							}
							$this->db->select(array(
								't1.type_detail_id',
								't1.type_id',
							...
							$this->db->order_by("start_date DESC");
							$arr_data[$key_rate]['interest_rate'] =  $this->db->get()->result_array();
							$arr_data[$key_rate]['day_of_year'] = $this->get_day_of_year_by_type($arr_data[$key_rate]['interest_rate'][$key_rate]['days_in_year'], $date_start);
						}else{
							$arr_data[$key_rate]['interest_rate'] = self::check_interest_rate($type_id, '', $date_start)['interest_rate'];
							$arr_data[$key_rate]['day_of_year'] = $this->get_day_of_year_by_type($arr_data[$key_rate]['interest_rate'][$key_rate]['days_in_year'], $date_start);
						}
					}	
				}
			
			}else if($main_type_id == 6){
				$balance = 0;		
				$arr_multi_rate = $this->calc_multi_step_rate($type_id,$date_start, $date_end, $balance,$chk_fee);
				if(!empty($arr_multi_rate)){
					foreach($arr_multi_rate AS $key_rate=>$val_rate){
			...
			if($check_year == 1){
				//$arr_one_step = $this->cal_int_one_step($date_start,$date_end,$date_middle,$type_id,$_date_start,$src_year,$chk_fee);
				//$arr_two_step = $this->cal_int_two_step($date_end,$date_middle,$type_id,$des_year,$chk_fee);				
				//$arr_data = array_merge($arr_one_step,$arr_two_step);
				
				$arr_one_step = $this->cal_int_one_step($date_start,$date_end,$date_middle,$type_id,$_date_start,$src_year,$chk_fee);
				
				$balance = 0;		
				$arr_multi_rate = $this->calc_multi_step_rate($type_id,$date_middle, $date_end, $balance,$chk_fee);
				if($arr_multi_rate){
					$arr_multi_step = $this->cal_int_multi_step($arr_multi_rate,$balance,$formula);
				}
				$arr_data = array_merge($arr_one_step,$arr_multi_step);
			}else{
				$arr_data = $this->cal_int_one_step($date_start,$date_end,$date_middle,$type_id,$_date_start,$src_year,$chk_fee);
			}
		}
		return $arr_data;
	}
	...
	function cal_int_one_step($date_start,$date_end,$date_middle,$type_id,$_date_start,$src_year,$chk_fee){
		$arr_data = array();
		$diff_one = @date_diff(date_create($date_start),date_create($date_middle));
		$date_count_one  = @$diff_one->format("%a");
		$arr_data[0]['date_count'] = $date_count_one;
	...
	function cal_int_two_step($date_end,$date_middle,$type_id,$des_year,$chk_fee){
		$arr_data = array();
		$diff_two = @date_diff(date_create($date_middle),date_create($date_end));
		$date_count_two  = @$diff_two->format("%a");
		$arr_data[1]['date_count'] = $date_count_two;
	...
	public function calc_multi_step_rate($type_id,$date_start, $date_end,$balance = 0,$chk_fee){
		if($chk_fee == 1){
			$field_interest_rate = "t1.percent_depositor as interest_rate";
		}else{
			$field_interest_rate = "t2.percent_interest as interest_rate";
            ...
            foreach ($term as $key => $res) {
                if (date_create($res['start_date']) <= date_create($date_start) || date_create($res['start_date']) <= date_create($date_end)) {
                   ...
                   // $data['start_date'] = date('Y-m-d', strtotime($res['start_date'] . " -1 day"));
					...
					$data['type_detail_id'] = $res['type_detail_id'];
                    $interest_rate[] = $data;
                }
            }
        }else{
		...
		//echo '<pre>'; print_r($interest_rate); echo '</pre>';
        return $interest_rate;
    }
	
	//คำนวณดอกเบี้ยหลาย step
	function cal_int_multi_step($arr_multi_rate, $formula = ''){
		$arr_data = array();
		$interest = 0;
		foreach($arr_multi_rate AS $key=>$val){			
			...
			$arr_data[$key]['date_count'] = $date_count;
			$arr_data[$key]['day_of_year'] = $day_of_year;
			$arr_data[$key]['interest_rate'][0]['type_detail_id'] = $val['type_detail_id'];
			$arr_data[$key]['interest_rate'][0]['type_id'] = $val['type_id'];
			$arr_data[$key]['interest_rate'][0]['start_date'] = $val['start_date'];
			$arr_data[$key]['interest_rate'][0]['days_in_year'] = $val['days_in_year'];
			$arr_data[$key]['interest_rate'][0]['num_month'] = $val['num_month'];
			$arr_data[$key]['interest_rate'][0]['interest_rate'] = $interest_rate;
			$arr_data[$key]['interest_rate'][0]['amount_deposit'] = $val['amount_deposit'];
	...
	*/
	...
	//เช็คปิดบัญชีก่อนกำหนด	
	function check_close_account_premature($arr_chk_fee){
		$result = false;
		//เช็คเงินฝากถอนก่อนกำหนด
		$this->db->select(array('created'));
		$this->db->from('coop_maco_account');
		$this->db->where("account_id = '".$arr_chk_fee['account_id']."'");
		$row_account = $this->db->get()->result_array();
		$row_account = $row_account[0];
		$create_date = date('Y-m-d',strtotime($row_account['created']));
		$end_date = date('Y-m-d',strtotime('+ '.$arr_chk_fee['num_month_before'].' month',strtotime($create_date)));
		
		if($arr_chk_fee['pay_interest'] == '4'){
			$chk_date_ext_maturity = $this->get_date_last_account($arr_chk_fee['account_id'],$arr_chk_fee['num_month_maturity'],$arr_chk_fee['ext_num_month_maturity_day']);
			if($chk_date_ext_maturity != ''){
				$end_date = date('Y-m-d',strtotime('+ '.$arr_chk_fee['ext_num_month_maturity_day'].' day',strtotime($chk_date_ext_maturity)));
			}
		}
		
		if($arr_chk_fee['date_interest'] < $end_date){
			$result = true;
		}else{
			$result = false;
		}
	...
	//วันที่ครบกำหนด due ล่าสุด
	function date_last_due($account_id,$ref_account_no){
		$row_last_due = $this->db->select('transaction_time')->from('coop_account_transaction')
							->where("account_id = '".$account_id."' AND ref_account_no = '".$ref_account_no."' AND transaction_list IN ('OPN', 'DFX')")
							->order_by("transaction_time DESC")->limit(1)->get()->row_array()['transaction_time'];
		return $row_last_due;
	}
	
	//เช็คข้อมูล ไม่คิดดอกเบี้ยในวันที่ทำที่มีรายการถอน
	function chk_date_withdraw($account_id,$date_interest){
		$result = false;
		$row = $this->db->select('transaction_time,transaction_balance')->from('coop_account_transaction')
							->where("account_id = '".$account_id."' AND transaction_time LIKE '".$date_interest."%' ")
