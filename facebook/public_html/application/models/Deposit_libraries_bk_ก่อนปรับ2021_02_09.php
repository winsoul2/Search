...
class Deposit_libraries extends CI_Model {
	...
	public function cal_deposit_interest($row_member,$return_type = 'test_cal_interest', $date_interest = '', $day_interest = '', $param_date_dep = '', $from_holiday = false){
		...
		elseif($row_detail['pay_interest'] == '4'){
			//$date_ext_maturity = $date_dep;
			//$date_ext_maturity = '2020-12-25';
			$where_chk = "";
			if($return_type == 'return_interest') {
				...
				/*$where_chk = " AND transaction_time <= DATE_ADD('".$date_ext_maturity." 23:59:59', INTERVAL -".$num_month_maturity." MONTH)".
					(empty($row_detail["max_month"]) ? "" : " AND transaction_time >= DATE_ADD('".$date_ext_maturity."', INTERVAL -".$row_detail["max_month"]." MONTH)")."
					AND DAY(transaction_time) = DAY(DATE_ADD('".$date_ext_maturity."', INTERVAL -".$num_month_maturity." MONTH))";
				*/	
			}
			...
			if($fix_saving) {
				...
				foreach($row_interest_rate as $key_rate => $value_rate){
					...
					if($value_rate['end_date'] != $date_interest && $date_interest > $value_rate['end_date']){
						...
						$this->db->where("account_id = '{$row_member['account_id']}' AND transaction_time < '{$value_rate['end_date']}' AND YEAR(transaction_time) = ".date("Y", strtotime("-1 year", strtotime($value_rate['end_date']))));
						$this->db->order_by("transaction_time DESC,transaction_id DESC");
						$this->db->limit(1);
				...
				foreach($transaction as $key => $value){
					...
					//$dd_test =date('Y-m-d', strtotime("+".$row_detail['ext_num_month_maturity_day']." day", strtotime($transaction_new[$key]['date_start'])));
					//echo 'date_dep='.$date_dep.'|'.$transaction[$key + 1]['date_start'].'|'.$dd_test.'<br>';
					//tuk
					if(empty($transaction[$key + 1]['date_start'])) {
                        if($return_type != 'return_interest') {
                            ...
                            $transaction_new[$i]['date_end'] = '2020-12-25';
...
//echo '<pre>'; print_r($transaction); echo '</pre>';
		//หาวันต้นปีและท้ายปี	
		$transaction = $this->get_transaction_start_year($transaction);
//echo '<pre>'; print_r($transaction); echo '</pre>';		
		foreach($transaction as $key => $value){
			...
			//tuk
			//echo 'key='.$key.'|'.$value['date_start'].','. $value['date_end'].'<br>';
	...
	*/
	...
	function get_transaction_start_year($transaction){
		$arr_data = array();
		$arr_date_start_year = Array
		(
			"0" => Array
				(
		...
		//echo '<pre>'; print_r($arr_date_start_year); echo '</pre>';		
		//echo '<pre>'; print_r($transaction); echo '</pre>';	
		$i=0;
		$transaction_new = array();
		foreach($transaction as $key => $value){
			...
			foreach($arr_date_start_year as $key2 => $value2){
				if(strtotime($transaction_new[$i]['date_start']) >= strtotime($value2['start_date']) && strtotime($transaction_new[$i]['date_start']) <= strtotime($value2['end_date'])){
					...
					if($value2['end_date'] != $cur_end_date && $cur_end_date != ""){
						if(strtotime($transaction_new[$i]['date_start']) != strtotime($value2['start_date']) && strtotime($transaction_new[$i - 1]['date_end']) != strtotime($value2['start_date'])){
							$is_split_rate = true;
							...
							$transaction_new[$i]['transaction_list'] = 'BF';								
				...
				if(strtotime($transaction_new[$i]['date_start']) == strtotime($value2['start_date'])){
					$transaction_new[$i]['transaction_list'] = 'BF';
				}	
			}
			$i++;
		}				
		$arr_data = $transaction_new;
