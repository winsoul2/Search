...
//				echo "<pre>"; print_r($transaction_new); exit;
		...
		foreach($transaction as $key => $value){
			...
			if($row_detail['is_withdrawal_specify'] == 1) {
				...
				$array_check_date = $this->check_interest_two_step($value['date_start'], $value['date_end'], $value['type_id'], $value['transaction_list'],$arr_chk_fee);
			}
	...
	function check_interest_two_step($date_start,$date_end, $type_id, $checkList = array(), $arr_chk_fee = array()){		
		$arr_data = array();
		$src_year = date("z", strtotime(substr($date_start, 0, 4)."-12-31")) + 1;
		$des_year = date("z", strtotime(substr($date_end, 0, 4)."-12-31")) + 1;
