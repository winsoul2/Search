...
class Loan_libraries extends CI_Model {
	...
	public function get_pre_contract_number($year, $loan_type)
	{
		...
		$this->db->order_by("convert(run_contract_number, decimal) DESC");
		$this->db->limit(1);
		...
		$data_insert['loan_name_id'] = $loan_name->loan_type_id;
    ...
    public function special_contract_number($year, $loan_type){

		$this->db->order_by('start_date', 'desc');
		$term = $this->db->get_where('coop_term_of_loan', array('type_id' => $loan_type), 1)->row_array();
		$prefix = $term['prefix_code'];

		if($prefix == "สพ."){
			$this->db->select('type_id');
			$this->db->from('coop_term_of_loan');
			$this->db->where("prefix_code LIKE 'สพ.'");
			$this->db->group_by('type_id');
			$typeArr= $this->db->get()->result_array();
			$typeList = array_column($typeArr, 'type_id');

			$this->db->select('run_contract_number');
			$this->db->from("coop_loan_contract_number");
			$this->db->where("contract_year = '".$year."' AND loan_type in (". implode(",", array_map(function ($data){ return sprintf("'%s'", $data); }, $typeList)).") ");
			$this->db->order_by("run_id DESC");
			$this->db->limit(1);
	...
	public function ignore_special_type(){
	}
	...
	public function cal_loan_interest($data,$return_type="echo",$type_count_date=""){
		...
		if(!empty($row)){
			foreach($row as $key => $value){
				$loan_transaction[$i]['loan_amount_balance'] = !empty($row_loan['loan_amount_balance']) ? $row_loan['loan_amount_balance'] : $value['loan_amount_balance'];
				if(!empty($last_payment_date)) {
					$loan_transaction[$i]['date_start'] = $last_payment_date;
				} else {
					$date_start = explode(' ',$value['transaction_datetime']);
    ...
    public function calc_interest_loan_type($loan_amount, $loan_type, $date1, $date2){
		...
		if($diff_y > 1){
			for ($i=0; $i < $diff_y; $i++) {
				...
				if($diff_y-1 == $i){
					$query = $this->db->query("select calc_loan_interest($loan_amount, $loan_type, '$c_date1', '$date2', 365) as interest");
				}else{
					$c_date2 = date("Y", strtotime($c_date1) ) . "-12-" . date("t", strtotime($c_date1) );
					$query = $this->db->query("select calc_loan_interest($loan_amount, $loan_type, '$c_date1', '$c_date2' , 365) as interest");
				}
			...
			$query = $this->db->query("select calc_loan_interest($loan_amount, $loan_type, '$date1', '$date2', 365) as interest");
			$tmp_interest = $query->result_array();
			$interest += array_sum(array_column($tmp_interest, 'interest'));
		}
		return $interest;
	...
	public function calc_interest_loan_type_with_loan_and_member_id($loan_amount, $loan_type, $date1, $date2, $member_id, $loan_id){
		$interest = 0;
		$d1 = date('Y', strtotime($date1));
		$d2 = date('Y',  strtotime($date2));
		$diff_y = $d2-$d1+1;
		...
		if ($diff_y > 1) {
			for ($i=0; $i < $diff_y; $i++) {
				$arg = array("member_id" => $member_id, "loan_id" => $loan_id);
				$dStart = new DateTime($c_date1);
				$interest_rate = 0;
				if ($diff_y-1 == $i) {
					$interest_rate = $this->Interest_modal->get_interest($loan_type, $date2, $arg);
					$dEnd  = new DateTime($date2);
			...
			$arg = array("member_id" => $member_id, "loan_id" => $loan_id);
			$interest_rate = $this->Interest_modal->get_interest($loan_type, $date2, $arg);
			$dStart = new DateTime(date("Y-m-d", strtotime($date1)));
			$dEnd  = new DateTime(date("Y-m-d", strtotime($date2)));
			$dDiff = $dStart->diff($dEnd);
			$days = $dDiff->format('%r%a');
			$interest += $loan_amount * $days * $interest_rate / 100 / 365;
		}
	...
	/**
	 * Count period of loan with transation
	 * @param int $id <p>
	 * is id form coop_loan
	 * @param date $date <p>
	 * date format date("Y-m-d")
	 * @return int
	 * </p>
	 */
    public function getPeriodCount($id, $date){
		$loan = $this->db->where(array('id' => $id))->get('coop_loan')->row_array();
		$_chk_date = $loan['date_last_interest'] ? $loan['date_last_interest'] : $loan['approve_date'];
		$_chk_date = date('Y-m-t', strtotime($_chk_date));

		$_period = ((int)$loan['period_now']) ? ((int)$loan['period_now'])+1 : 1;
		if($_chk_date >= $date){
			return $_period;
		}else{
			return $_period+1;
	 ...
	 * @return string|null
	 */
	public function getLoanName($id)
	{
		$loan_type = self::getLoanById($id)->loan_type;

		if($loan_type == ""){
			return null;
		}

		return $this->db->select('loan_name')
				->where(array('loan_name_id' => $loan_type))
				->get('coop_loan_name', 1)
				->row()->loan_name;
	}

	/**
	 * Get contect type name
	 * @param string $id <b>defualt</b> is empty
	 * @return string|null <b>defualt</b> is null
	 */
	public function getLoanTypeNameByLoanId($id = "")
	{
		...
		return $this->db->select('t1.loan_type')
			->from("coop_loan_type as t1")
			->join('coop_loan_name as t2', 't1.id=t2.loan_type_id', 'inner')
			->where(array('t2.loan_name_id' => $loan_type))
			->get()->row()->loan_type;
	}
	...
	public function ConvertLoanNameShort($type_id){
		$rs = $this->db->select('prefix_code')
