...
foreach($account_chart_main as $key => $value){
    if(!empty($data[$key])){
		...
		$pdf->MultiCell(140, 8, U2T("ชื่อบัญชี ".$account_chart_main[$key]), 0, "L");
		...
		foreach($data[$key] as $value_detail) {
			...
			$pdf->MultiCell($col_w_2, 7, U2T($value_detail["journal_ref"]."-".$value_detail["seq_no"]), 0, "C");
			$pdf->SetFont('THSarabunNew', '', $font_size);
			$pdf->SetXY($col_x_3, $y_point);
			$pdf->MultiCell($col_w_3, 7, U2T($value_detail["account_chart"]), 0, "L");
			$h = $pdf->GetY();
			$pdf->SetXY($col_x_4, $y_point);
