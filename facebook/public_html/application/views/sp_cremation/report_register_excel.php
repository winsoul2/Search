...
$title_date = "";
if(!empty($_POST["register_from_date"])) $title_date .= "วันที่ ".$this->center_function->ConvertToThaiDate($this->center_function->ConvertToSQLDate($_POST['register_from_date']));
if(!empty($_POST["register_thru_date"]) && $_POST["register_from_date"] != $_POST["register_thru_date"]) $title_date .= "ถึงวันที่ ".$this->center_function->ConvertToThaiDate($this->center_function->ConvertToSQLDate($_POST['register_thru_date']));
...
$i++;
$objPHPExcel->getActiveSheet()->mergeCells('A'.$i.':F'.$i);
$objPHPExcel->getActiveSheet()->SetCellValue('A' . $i , "รายงานการสมัครสมาชิก"); 
$objPHPExcel->getActiveSheet()->getStyle('A'.$i)->applyFromArray($titleStyle);
$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':F'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$objPHPExcel->getActiveSheet()->SetCellValue('F' . $i , "วันที่เป็นสมัคร");
...
foreach($datas as $index=>$data) {
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('E' . $i , $status[$data["reason"]]);
...
header('Content-Disposition: attachment;filename="รายงานการสมัครสมาชิกฌาปนกิจ '.$cremation["name"].'.xlsx"');
