...
foreach($datas as $index=>$data) {
	if($index == 0 || (($y_point + 42) > 297)) {
		...
		$y_point += 10;
		$pdf->SetXY( 0, $y_point );
		$pdf->MultiCell(210, 8, U2T("รายงานการสมัครสมาชิก"), 0, "C");
		...
		$pdf->MultiCell($col_w_7, $line_height, U2T("วันที่สมัคร"), 1, "C");
		$y_point += $line_height;
	}
	...
	$y = $y < $pdf->GetY() ? $pdf->GetY() : $y;
	$pdf->SetXY($col_x_7, $y_point);
	$pdf->MultiCell($col_w_7, $line_height, U2T($this->center_function->ConvertToThaiDate($data["request_date"],'1','0')), 0, "C");
	$y = $y < $pdf->GetY() ? $pdf->GetY() : $y;
