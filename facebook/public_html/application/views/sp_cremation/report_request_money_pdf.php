...
foreach($datas as $index=>$data) {
	if($index == 0 || (($y_point + 42) > 297)) {
		...
		$y_point += 10;
		$pdf->SetXY( 0, $y_point );
		$pdf->MultiCell(210, 8, U2T("รายงานการขอรับเงินฌาปนกิจ"), 0, "C");
		...
		$pdf->MultiCell($col_w_5, $line_height, U2T("สาเหตุการเสียชีวิต"), 1, "C");
		$pdf->SetXY($col_x_6, $y_point);
		$pdf->MultiCell($col_w_6, $line_height, U2T("สถานะ"), 1, "C");
		$pdf->SetXY($col_x_7, $y_point);
		$pdf->MultiCell($col_w_7, $line_height, U2T("วันที่ขอลาออก"), 1, "C");
		$y_point += $line_height;
	}
	...
	$y = $pdf->GetY();
	$pdf->SetXY($col_x_5, $y_point);
	$pdf->MultiCell($col_w_5, $line_height, U2T($data["reason"]), 0, "L");
