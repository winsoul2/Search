...
<div class="layout-content">
    <div class="layout-content-body">
		...
		<h1 style="margin-bottom: 0">รายงานสมาชิก</h1>
		<?php $this->load->view('breadcrumb'); ?>
		<div class="row gutter-xs">
			<div class="col-xs-12 col-md-12">
				<div class="panel panel-body" style="padding-top:0px !important;">
				<form action="<?php echo base_url(PROJECTPATH.'/sp_cremation/'.$path.'/member_pdf'); ?>" id="form1" method="POST" target="_blank">
					<h3></h3>
...
<script>	
	...
	$( document ).ready(function() {
        $("#submit_btn").click(function() {
            $.ajax({
                url: base_url+'/sp_cremation/<?php echo $path;?>/check_member_report',	
                method:"post",
                ...
                success:function(result){
                    ...
                    if(data["status"] == 'success'){
						$('#form1').attr('action', base_url+'/sp_cremation/<?php echo $path;?>/member_pdf');
                        $('#form1').submit();
                    } else {
		...
		$("#submit_excel_btn").click(function() {
            $.ajax({
                ...
                success:function(result){
                    data = JSON.parse(result);
                    if(data["status"] == 'success'){
						$('#form1').attr('action', base_url+'/sp_cremation/<?php echo $path;?>/member_excel');
                        $('#form1').submit();
                    } else {
                        $('#alertNotFindModal').appendTo("body").modal('show');
                    }
